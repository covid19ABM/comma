        ...
        for file in initial_csv_data["csv_files"][1:]:
            ...
            if list(df.columns) != list(initial_csv_data["column_names"]):
                diff_order = [i for i, (col_df, col_base) in
                              enumerate(zip(df.columns,
                                            initial_csv_data["column_names"])
                                        ) if
                              col_df != col_base]
                raise AssertionError(
                    f"{file} has different column "
                    f"order at indices: {diff_order}."
                )

            # Check for the same "actions" entries
            assert set(
                df['actions'].unique()) == initial_csv_data["actions"], \
