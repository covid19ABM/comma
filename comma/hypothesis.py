...
class Hypothesis:
    """
    The Hypothesis class is responsible for managing and validating the Hypotheses specified by the user.
    These can be found in the `dir_params` folder.

    Methods:
        _get_one_hot_encoded_features(cls, fpath_params_individual: str) -> list:
            One-hot encodes categorical features in the `params_individual.json` file and returns the feature list.

        create_empty_hypotheses(dir_params: str):
            Creates empty CSV files for storing hypotheses on the impact of actions and lockdown policies on different agent statuses.

        validate_param_file(dir_params: str):
            Validates the files in the parameter folder, ensuring that all necessary files and parameters are present and correctly formatted.
    ...
    def _get_one_hot_encoded_features(fpath_params_individual: str):
        ...
        Args:
            fpath_params_individual (str): Path to the individual parameters JSON file.
    ...
    def create_empty_hypotheses(cls, dir_params: str):
        ...
        if not os.path.exists(fpath_params_individual):
            raise FileNotFoundError(f"'{PARAMS_INDIVIDUAL}' file is missing in the directory '{dir_params}'")
        if not os.path.exists(fpath_params_model):
            raise FileNotFoundError(f"'{PARAMS_MODEL}' file is missing in the directory '{dir_params}'")
    ...
    def validate_param_file(cls, dir_params: str):
        ...
        missing_model_params = set(cls._required_params) - set(params_model.keys())

        if missing_model_params:
            raise ValueError("Model parameter(s) not found: %s." % ", ".join(missing_model_params))
        ...
        if any(overlaps):
            raise ValueError("Lockdown intervals have overlap: \n%s" % "\n".join(
        ...
        required_features += cls._get_one_hot_encoded_features(fpath_params_individual)
