...
class Model:
    ...
    def update(self, lockdown: str, step: int):
        ...
        if self.current_step == 0:
            ...
            self.cumulative_status[step] = [(lockdown, agent.id, delta_mh, agent.get_status()) for agent in self.agents]
            ...
            agent_statuses = []
            for agent in self.agents:
                last_status = [status for status in self.cumulative_status[step - 1] if status[1] == agent.id][0][-1]
